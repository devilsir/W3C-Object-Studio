/*
 * JPEG decoder core derived from notmasteryet/pdf.js-era code.
 * Copyright 2011 notmasteryet
 * Licensed under the Apache License, Version 2.0.
 *
 * Warcraft III BLP1 adaptation in WC3 Asset Studio:
 * Blizzard BLP1 JPEG payloads may store four raw components as B,G,R,A.
 * This wrapper intentionally bypasses generic JPEG CMYK/YCC conversion and
 * preserves the fourth component as alpha.
 */
(function (global) {
  'use strict';
...
break;default:throw new Error('Unsupported color mode');}}};return constructor;})();

function wc3DecodeBlpJpeg(data) {
  if (!(data instanceof Uint8Array)) data = new Uint8Array(data);
  var jpeg = new JpegImage();
  jpeg.parse(data);
  var width = jpeg.width;
  var height = jpeg.height;
  var comps = jpeg.components || [];
  if (!width || !height || !comps.length) throw new Error('BLP JPEG decoder produced no image components.');
  var rgba = new Uint8ClampedArray(width * height * 4);
  var scaleX = jpeg.width / width;
  var scaleY = jpeg.height / height;

  function sample(comp, x, y) {
    var yy = Math.max(0, Math.min(comp.lines.length - 1, Math.floor(y * comp.scaleY * scaleY)));
    var line = comp.lines[yy];
    var xx = Math.max(0, Math.min(line.length - 1, Math.floor(x * comp.scaleX * scaleX)));
    return line[xx];
  }
  ...
  for (var y = 0; y < height; y++) {
    for (var x = 0; x < width; x++) {
      var di = (y * width + x) * 4;
      if (n >= 3) {
        // BLP1 raw JPEG component order used by Warcraft III is B, G, R, [A].
        rgba[di] = sample(comps[2], x, y);
        rgba[di + 1] = sample(comps[1], x, y);
        rgba[di + 2] = sample(comps[0], x, y);
        rgba[di + 3] = n >= 4 ? sample(comps[3], x, y) : 255;
      } else if (n === 2) {
        var gray = sample(comps[0], x, y);
        rgba[di] = gray;
        rgba[di + 1] = gray;
        rgba[di + 2] = gray;
        rgba[di + 3] = sample(comps[1], x, y);
      } else {
        var g = sample(comps[0], x, y);
        rgba[di] = g;
        rgba[di + 1] = g;
        rgba[di + 2] = g;
        rgba[di + 3] = 255;
  ...
  return { width: width, height: height, data: rgba, components: n };
}

global.WC3BLPJpeg = { decode: wc3DecodeBlpJpeg };
